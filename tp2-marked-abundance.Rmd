...
On utilise "share = TRUE" pour une seule probabilité de capture. 
```{r}
run.mouse <- function() {
...
Run the models and examine the output, and examine model-selection table
```{r}
...
examine the output from top-ranked model (#9)
```{r}
...
run.mouse <- function() {
  ...
  # avec l'effet age
  p.age <- list(formula =  ~ age, share = TRUE)
  p.age.behav <- list(formula =  ~ age)
  p.time.age <- list(formula =  ~ time + age, share = TRUE)
  p.time.behav.age <- list(p = list(formula =  ~ age + time), 
                           c = list(formula =  ~ age + time))
  p.h.age <- list(formula = ~ mixture + age, share = TRUE)
  p.h.behav.age <- list(p = list(formula =  ~ age + mixture), 
                        c = list(formula =  ~ age + mixture))
  p.h.time.age <- list(formula = ~ time + mixture + age, share = TRUE)
  p.h.time.behav.age <- list(p = list(formula =  ~ age + mixture + time), 
                             c = list(formula =  ~ age + mixture + time))
...
examine the output from top-ranked model (#3)
```{r}
...
On construit les histoires de capture.
```{r}
histoires <- dat %>% 
...
```{r}
monarque.proc <- process.data(monarque_reduced, 
                              begin.time = 1, 
                              model = "FullHet")
```
