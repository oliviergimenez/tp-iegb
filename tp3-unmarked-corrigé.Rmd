---
title: "TP 3 : analyses des données sur animaux non-marqués"
output:
...
```{r}
temps <- matrix(c('tps 1','tps 2','tps 3'),
...
```{r}
nd <- data.frame(temps = c('tps 1','tps 2','tps 3'))
...
Et pour l'occupancy, on crée d'abord une grille de valeurs pour le nombre de terriers sur laquelle on va calculer cette probabilité.
```{r}
...
On calcule la probabilité d'occupancy pour chaque valeur de terriers.
```{r}
...
On a refait le jeu de données à chaque fois, mais on aurait pu mettre toutes les covariables en une fois.
...
fm1 <- pcount(~ temps ~ 1, lezard, K = 100)
fm2 <- pcount(~ 1 ~ lapin, lezard, K = 100)
fm3 <- pcount(~ temp ~ 1, lezard, K = 100)
fm4 <- pcount(~ temps ~ lapin, lezard, K = 100)
```
...
Le jeu de données formaté pour unmarked. 
```{r}
...
Le modèle avec paramètres tous constants.
```{r}
...
On crée un variable pour l'effet du temps, et on prédit colonisation et extinction. 
```{r}
